...
module tb_top_riscv_single;
  ...
  wire [31:0] pc;
  wire [31:0] read_data, mem_wr_addr, mem_wr_data;

 // Instantiate the top module
  top_riscv_single #(32) dut (
    .clk(clk),
    .reset(reset),
    .result(result),
    .pc(pc),
...
// end
  initial begin
    ...
    $display("Testing %0d-bit implementation\n", WIDTH);
...
always @(negedge clk) begin
    if(reset) begin
      case(dut.pc)
        ...
        SB      : begin
            i = i + 1'b1;
            //if(MemWrite && !reset) begin
                if(dut.mem_wr_addr === 33 & dut.mem_wr_data === 1) $display ("23. sb implementation is correct");
                else begin
        ...
        SH      : begin
            i = i + 1'b1;
            //if(MemWrite && !reset) begin
                if(dut.mem_wr_addr === 38 & dut.mem_wr_data === -3) $display ("24. sh implementation is correct");
                else begin
        ...
        SW      : begin
            i = i + 1'b1;
            //if(MemWrite && !reset) begin
                if(dut.mem_wr_addr === 40 & dut.mem_wr_data === 16) $display ("25. sw implementation is correct");
                else begin
        ...
        LB      : begin
            i = i + 1'b1;
            if(dut.mem_wr_addr === 33 & result === 1 ) $display ("26. lb implementation is correct");
            else begin
        ...
        LH      : begin
            i = i + 1'b1;
            if(dut.mem_wr_addr === 38 & result === -3 ) $display ("27. lh implementation is correct");
            else begin
        ...
        LW      : begin
            i = i + 1'b1;
            if(dut.mem_wr_addr === 40 & result === 16) $display ("28. lw implementation is correct");
            else begin
        ...
        LBU      : begin
            i = i + 1'b1;
            if(dut.mem_wr_addr === 33 & result === 1) $display ("29. lbu implementation is correct");
            else begin
        ...
        LHU     : begin
            i = i + 1'b1;
            if(dut.mem_wr_addr === 38 & result === 32'h0000FFFD) $display ("30. lhu implementation is correct");
            else begin
 ...
 // Final summary block
  always @(negedge clk) begin
    if ((i >= 38 || flag == 1) && !reset) begin
      #20;
      $display("Total Instructions Tested: %0d", i);
...
//     #5000;
//     if (i < 34) begin
//       $display("\nWARNING: Simulation timeout - only %0d tests completed", i);
//     end
//     $finish;
//   end
//
...
/*
initial begin
  $monitor("Time: %0t | PC: 0x%h | Result: 0x%h (%0d)",
           $time, dut.pc, result, result_signed);
end*/
